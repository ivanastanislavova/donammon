...
\item[RF3:] Al acercarse físicamente a un punto geolocalizado, la aplicación debe desbloquear el contenido de RA asociado.
...
\item[RF6:] La app debe permitir visualizar elementos de realidad aumentada (como imágenes, objetos 3D o figuras animadas).
...
\item[RF13:] El usuario debe disponer de un historial donde se registre la fecha y hora de cada punto visitado.
\item[RF14:] El usuario debe disponer de un historial donde se registre la fecha y hora de cada ruta completada.
...
\item[RF19:] El administrador debe poder gestionar áreas de investigación y rutas temáticas desde el panel de administración. 
\item[RF20:] La experiencia de RA debe ofrecer retroalimentación visual y sonora.
\item[RF21:] El sistema debe enviar datos al backend para mantener sincronizado el progreso del usuario.
\item[RF22:] La aplicación debe permitir a los usuarios registrarse y acceder mediante un sistema de inicio de sesión.
\item[RF23:] Los usuarios deben poder iniciar sesión con un nombre de usuario y contraseña.
\item[RF24:] La aplicación debe mantener la sesión iniciada hasta que el usuario decida cerrarla.
\item[RF25:] El usuario debe poder editar los datos de su perfil desde la aplicación.
\item[RF26:] El administrador debe poder acceder, desde el panel de gestión, a la lista de usuarios registrados.
\item[RF27:] Al iniciar la aplicación, debe mostrarse una pantalla de bienvenida (splash screen) con el logotipo oficial antes de acceder a la pantalla principal.
...
\item[RF42:] El usuario debe poder consultar una sección de perfil donde ver su historial de lugares visitados y rutas completadas, y cerrar sesión.
\item[RF43:] El sistema debe implementar un contexto global de filtros que permita mantener el estado entre diferentes pantallas.
\item[RF44:] La aplicación debe calcular distancias precisas utilizando la fórmula de Haversine\cite{viafirmaHaversine}.
\item[RF45:] El mapa debe sincronizar automáticamente los filtros aplicados en la pantalla de listado.
\item[RF46:] El sistema debe permitir visualizar imágenes ampliadas en modal desde la pantalla de detalle.
\item[RF47:] La aplicación debe procesar automáticamente las áreas de investigación del backend para crear una estructura plana de lugares.
\item[RF48:] El sistema debe mantener un cache local de estado de visitas utilizando AsyncStorage.
\item[RF49:] La aplicación debe mostrar indicadores visuales de filtros activos en tiempo real.
\item[RF50:] El sistema debe permitir reiniciar el progreso de rutas completadas.
\item[RF51:] El usuario debe poder buscar mujeres y lugares por nombre en tiempo real.
\end{itemize}
...
\begin{itemize}
    ...
    \item[RNF2:] El sistema debe ser capaz de gestionar al menos 50 usuarios simultáneos sin caída de rendimiento, con capacidad estimada para 100+ usuarios. 
    \item[RNF3:] Los modelos 3D deben estar optimizados para dispositivos móviles, con un tamaño inferior a 20~MB por unidad.
    \item[RNF4:] La aplicación debe implementar optimizaciones de rendimiento como cache local de tokens, serialización eficiente de datos y consultas optimizadas al backend para minimizar el uso de recursos.
    ...
    \item[RNF7:] La aplicación debe ofrecer un modo accesible con audio narrado.
    ...
    \item[RNF13:] El backend debe estar desarrollado en Django y utilizar PostgreSQL como sistema gestor de bases de datos.
    \item[RNF14:] La experiencia de realidad aumentada debe estar desarrollada con A-Frame y AR.js, desplegadas en Vercel e integradas a través de WebViews nativos.
...
\begin{itemize}
    \item[RNF17:] El sistema debe permitir su despliegue en un servidor que soporte Django y PostgreSQL.
    \item[RNF18:] La aplicación utiliza el proveedor de mapas nativo del dispositivo (Google Maps en Android, Apple Maps en iOS).
...
\begin{itemize}
    \item[RNF21:] El acceso a los datos de usuario debe estar restringido y protegido contra accesos no autorizados.
    \item[RNF22:] El almacenamiento de datos personales (nombre, email, localización) debe hacerse de forma segura.
    \item[RNF23:] El sistema debe mantener un throughput mínimo de 8 requests por segundo bajo carga normal. 
    \item[RNF24:] Los endpoints de la API deben responder en menos de 100ms bajo condiciones normales de uso.
\end{itemize}
...
\begin{itemize}
    ...
    \item[F3.] Calcular y mostrar la distancia a cada punto desde la ubicación actual del usuario.
    \item[F4.] Notificar al usuario cuando se acerque a un punto no visitado.
    \item[F5.] Mostrar puntos cercanos en función de la ubicación.
\end{itemize}
...
\begin{itemize}
    ...
    \item[F8.] Consultar un listado general de todas las mujeres incluidas en la app.
    \item[F9.] Visualizar imágenes ampliadas en modal al hacer tap sobre ellas.
    \item[F10.] Compartir información de lugares vía aplicaciones nativas del dispositivo.
\end{itemize}
...
\begin{itemize}
    \item[F11.] Registrar un nuevo usuario mediante nombre de usuario y contraseña.
    \item[F12.] Iniciar sesión en la aplicación.
    \item[F13.] Mantener la sesión activa entre usos.
    \item[F14.] Permitir al usuario editar su nombre de usuario.
    \item[F15.] Cerrar sesión manualmente.
    \item[F16.] Filtrar lugares por área de investigación y estado de visita.
    \item[F17.] Buscar mujeres y lugares por nombre en tiempo real.
    \item[F18.] Sincronizar estado de filtros entre diferentes pantallas de la aplicación.
    \item[F19.] Consultar el historial de puntos visitados, con fecha y hora.
    \item[F20.] Consultar el historial de rutas completadas, con fecha y hora.
\end{itemize}
...
\begin{itemize}
    \item[F21.] Iniciar sesión como administrador desde el backend web.
    \item[F22.] Añadir nuevas figuras históricas con nombre, descripción y material multimedia.
    \item[F23.] Asociar cada mujer con un punto geolocalizado.
    \item[F24.] Cargar imágenes, audios, vídeos y modelos 3D desde el panel.
    \item[F25.] Editar o eliminar puntos o figuras ya creadas.
    \item[F26.] Ver la lista de usuarios registrados.
...
\begin{itemize}
    \item[F28.] Adaptar la interfaz a distintos tamaños de pantalla.
    \item[F29.] Navegar por la app de forma intuitiva y con accesibilidad mobile-first.
    \item[F30.] Garantizar el uso fluido y sin errores tanto en Android como en iOS.
\end{itemize}

\vspace{0.3cm}

\textbf{\textsf{\large Funcionalidades relacionadas con rutas y QRs}}

\vspace{0.3cm}

\item[F31.] Ver una lista de rutas temáticas con sus nombres y descripciones.
\item[F32.] Consultar los puntos que forman parte de cada ruta.
\item[F33.] Escanear códigos QR para desbloquear puntos de una ruta.
\item[F34.] Reiniciar el progreso de rutas ya completadas para repetir la experiencia.

\vspace{0.3cm}

\textbf{\textsf{\large Funcionalidades de realidad aumentada (RA)}}

\vspace{0.3cm}

\item[F35.] Ver una lista de los puntos con contenido RA disponible.
\item[F36.] Mostrar únicamente los puntos con RA que estén a una distancia determinada del usuario.
\item[F37.] Al seleccionar uno de estos puntos, abrir automáticamente la cámara y cargar el modelo RA asociado.
\vspace{0.3cm}
...
\subsection{Ventajas del modelo en cascada}
\begin{itemize}
...
La fecha de inicio del proyecto fue el \textbf{3 de marzo de 2025}, y la fecha de finalización fue el \textbf{20 de junio de 2025}.
\clearpage
\begin{figure}[H]
    \centering
