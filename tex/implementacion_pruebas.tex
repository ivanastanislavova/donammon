...
\begin{itemize}
    ...
    \item \textbf{Troika Text} - Renderizado avanzado de texto 3D en WebGL para A-Frame
...
\begin{lstlisting}[language=HTML, caption={Gestor de transiciones AR}]
...
Este sistema implementa una \textbf{temporización precisa} que controla cada fase de la experiencia, asegurando transiciones fluidas entre estados. 


\subsubsection{Arquitectura de audio multicapa}

En el código del apartado anterior, también se muestra la \textbf{gestión de audio inteligente} que reduce automáticamente el volumen de la música ambiental durante las narraciones, restaurándolo progresivamente al finalizar. Para la música ambiental se ha escogido la ``Canción sin miedo'' de Vivir Quintana \cite{wpJn20WoEWM} y las narraciones son audios propios sobre las mujeres y sus lugares asociados, que se reproducen al mostrarse el modelo de la mujer.
...
\begin{lstlisting}[language=Python, caption={Gestión de autoplay de audio}]
...
<a-entity id="infoPanel"
    ...
    visible="false">
    <a-entity troika-text="value: Su compromiso con la igualdad y la cultura la llevó a dirigir el Instituto Valenciano de Arte Moderno (IVAM) entre 1988 y 1993, etapa en la que el museo se consolidó como referente nacional e internacional del arte contemporáneo. Desde allí impulsó una programación abierta, crítica y sensible al papel de las mujeres en la creación artística.; 
...
\begin{enumerate}
    ...
    \item Generándolos mediante inteligencia artificial usando la herramienta Meshy\footnote{\url{https://www.meshy.ai}}. Este proceso consiste en subir una imagen, obtener cuatro modelos generados automáticamente y seleccionar el más adecuado. Posteriormente, Meshy crea una textura para el modelo elegido (como se observa en la figuras \ref{fig:meshy1}, \ref{fig:meshy2} y \ref{fig:manuela_solis}).
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figs/meshy1.jpg}
    \caption{Cuatro modelos generados automáticamente por Meshy a partir de una imagen de entrada.}
    \label{fig:meshy1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figs/meshy2.jpg}
    \caption{Modelo seleccionado de los cuatro generados, ya con la textura aplicada por Meshy.}
    \label{fig:meshy2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{figs/manuela_solis.jpg}
    \caption{Imagen original de Manuela Solís que se utiliza como entrada en Meshy para la generación automática de modelos 3D.}
    \label{fig:manuela_solis}
\end{figure}

\textbf{Optimización y limitaciones técnicas}

Durante el desarrollo se presentaron varias restricciones de tamaño. La herramienta Meshy solo admite modelos de hasta 50\,MB, lo que obligó a realizar procesos de optimización y reducción de malla (re‑vectorización) antes de subirlos para generar las texturas. Asimismo, la plataforma de despliegue Vercel establece un límite máximo recomendado de 10\,MB por archivo para proyectos conectados por repositorios GitHub \cite{vercel_limits}, como es nuestro caso. Esto exigió la aplicación de técnicas adicionales de compresión y simplificación para asegurar la compatibilidad.

Finalmente, en algunos casos los modelos descargados desde repositorios carecían de texturas, por lo que se recurrió a Meshy para generarlas. Esto implicó un flujo de trabajo iterativo que combinaba conversión a \texttt{.glb}, reducción de tamaño y texturizado mediante IA.

\subsection{Implementación del sistema de tracking en AR}

Como se ha mencionado anteriormente, la experiencia de realidad aumentada se implementó usando \textbf{A-Frame} junto con la librería \textbf{AR.js}. Esta combinación permite integrar modelos 3D en entornos web mediante tecnologías abiertas como WebGL y WebRTC, sin requerir aplicaciones nativas. No obstante, presenta una limitación fundamental: \textbf{AR.js no incorpora seguimiento posicional basado en el entorno (SLAM)}, sino que se apoya en \textbf{marcadores visuales} o en coordenadas GPS para determinar la posición del contenido en la escena. Como consecuencia, no es posible anclar los objetos 3D a superficies reales ni proporcionar una experiencia completamente inmersiva con movimiento libre.

Una solución a esta carencia es el uso de la \textbf{API WebXR}, que ofrece soporte nativo para AR con capacidades de seguimiento posicional en navegadores modernos. Sin embargo, esta alternativa presenta un problema de compatibilidad relevante: actualmente, \textbf{Safari en iOS no soporta la API WebXR}, lo que imposibilita su uso en dispositivos Apple \cite{mdn_webxr,caniuse_webxr}. Este aspecto limita su adopción en escenarios donde se busca garantizar la disponibilidad multiplataforma.

Ante esta restricción, se diseñó un mecanismo alternativo que simula el comportamiento del anclaje espacial, como un \textit{tracking visual simulado}. Este enfoque consiste en mantener el modelo 3D en una posición fija dentro de la escena virtual, aplicando transformaciones relativas a la cámara. De este modo, al rotar el dispositivo, el objeto parece permanecer en un punto del espacio, generando la \textbf{sensación de anclaje} aunque en realidad no se esté realizando un seguimiento del entorno. Si bien este método no sustituye al SLAM real, permite mejorar la percepción de estabilidad en la experiencia y reduce la disonancia visual en comparación con un objeto flotante sin referencia.
