...
La aplicación implementa cálculos de distancia precisos mediante la \texttt{fórmula de Haversine}\cite{viafirmaHaversine}, que permite determinar la distancia entre la ubicación del usuario y cada lugar en la superficie terrestre:
...
Para la implementación de las experiencias de realidad aumentada se ha optado por una solución web basada en \textbf{A-Frame} con \textbf{AR.js}, que permite crear experiencias inmersivas accesibles desde cualquier navegador web moderno sin necesidad de aplicaciones nativas específicas.

Las principales tecnologías utilizadas son:
\begin{itemize}
    \item \textbf{A-Frame 1.7.0} - Framework web para realidad virtual y aumentada
    \item \textbf{AR.js} - Librería JavaScript que añade capacidades de realidad aumentada a A‑Frame 
...
Cada experiencia AR se aloja en una URL única, por ejemplo: \texttt{https://ivam.vercel.app/}. Esta URL se asocia a un lugar específico en la base de datos del backend, como hemos mencionado anteriormente.
...
\subsection{Gestión de la tipografía en textos 3D}

Durante la implementación de los elementos textuales en la escena 3D, se identificó una limitación en el componente nativo \texttt{<a-text>} de A-Frame, el cual no soporta caracteres extendidos como acentos o la letra “ñ” debido a que utiliza fuentes basadas en texturas MSDF con un conjunto restringido de caracteres. Esta restricción impedía mostrar correctamente textos en castellano, incluso cuando la codificación del documento se establecía en UTF-8 \cite{milewski2018_stackoverflow_utf8_aframe}. 

Para solventar este problema, se incorporó el componente \texttt{aframe-troika-text}, que permite renderizar tipografía en tiempo real a partir de fuentes en formatos estándar (TTF o WOFF) y ofrece soporte completo para caracteres Unicode. Esto se consigue añadiendo la librería mediante la siguiente instrucción en el documento HTML:

\begin{lstlisting}[language=HTML, caption={Inclusión del componente aframe-troika-text.}]
<script src="https://unpkg.com/aframe-troika-text/dist/aframe-troika-text.min.js"></script>
\end{lstlisting}

Posteriormente, se definió un panel informativo en la escena con un elemento \texttt{<a-entity>} que emplea el componente \texttt{troika-text}, configurando propiedades como el texto, la fuente personalizada (en este caso, \textit{Roboto} en formato TTF), el color, el tamaño y el ancho máximo. El siguiente fragmento de código muestra la implementación:

\begin{lstlisting}[language=HTML, caption={Uso de aframe-troika-text para mostrar texto con acentos.}]
<!-- Panel informativo -->
<a-entity id="infoPanel" geometry="primitive: plane; height: 1; width: 2"
  material="color: #f8d4e6; opacity: 0.8" position="0 -0.036 -3.99313"
  visible="false" troika-text="
    value: Su compromiso con la igualdad y la cultura la llevó a dirigir el Instituto Valenciano de Arte Moderno (IVAM) entre 1988 y 1993;
    font: ./fonts/Roboto-Regular.ttf;
    color: #33C3F0;
    fontSize: 0.13;
    maxWidth: 1.8;
    align: left
" position="-0.8571 0.046 0.08175">
</a-entity>
\end{lstlisting}

Gracias a esta solución, se logró una visualización correcta y estilizada de los textos en español dentro de la experiencia de realidad aumentada.



\subsection{Formato y optimización de modelos 3D}

Todos los modelos empleados en la experiencia de realidad aumentada se han utilizado en formato \texttt{.glb}. Este formato, perteneciente al estándar glTF 2.0, es uno de los soportados por A‑Frame mediante el componente \texttt{gltf-model} \cite{aframe_models}. A‑Frame también permite el uso de otros formatos como \texttt{.obj} (a través del componente \texttt{obj-model}) y, mediante loaders de \textit{three.js}, formatos adicionales como \texttt{.ply}, \texttt{.fbx} o JSON. Sin embargo, la documentación oficial recomienda el uso de glTF/GLB por sus ventajas: encapsula geometría, texturas, materiales y animaciones en un único archivo binario, optimizado para transmisión web, reduciendo el número de peticiones HTTP y facilitando la carga en dispositivos móviles.

\textbf{Motivos para la elección del formato \texttt{.glb}}:
\begin{itemize}
    \item Compatibilidad nativa con A‑Frame.
    \item Inclusión de texturas y materiales en un único archivo, evitando dependencias adicionales como ocurre en \texttt{.obj}.
    \item Rendimiento optimizado en entornos web y móviles.
\end{itemize}

\textbf{Obtención de modelos}: los modelos utilizados se han conseguido de dos formas:
\begin{enumerate}
    \item Descargando desde repositorios especializados como Cults3D\footnote{\url{https://cults3d.com/en/3d-model/art/vandana-shiva?srsltid=AfmBOoo2OFfWztX_LpR_AyTC21vPPs9T8Xd-n4IRsIl4syxbvKVhqBf3}} y Thingiverse\footnote{\url{https://www.thingiverse.com/thing:5899901}}, que ofrecen modelos en diferentes formatos posteriormente convertidos a \texttt{.glb}.
    \item Generándolos mediante inteligencia artificial usando la herramienta Meshy\footnote{\url{https://www.meshy.ai}}. Este proceso consiste en subir una imagen, obtener cuatro modelos generados automáticamente y seleccionar el más adecuado. Posteriormente, Meshy crea una textura para el modelo elegido.
\end{enumerate}
...
\textbf{Optimización y limitaciones técnicas}: durante el desarrollo se presentaron varias restricciones de tamaño. La herramienta Meshy solo admite modelos de hasta 50\,MB, lo que obligó a realizar procesos de optimización y reducción de malla (re‑vectorización) antes de subirlos para generar las texturas. Asimismo, la plataforma de despliegue Vercel establece un límite máximo recomendado de 100\,MB por archivo para proyectos en planes estándar \cite{vercel_limits}. Esto exigió la aplicación de técnicas adicionales de compresión y simplificación para asegurar la compatibilidad.

Finalmente, en algunos casos los modelos descargados desde repositorios carecían de texturas, por lo que se recurrió a Meshy para generarlas. Esto implicó un flujo de trabajo iterativo que combinaba conversión a \texttt{.glb}, reducción de tamaño y texturizado mediante IA, garantizando así la integración con A‑Frame y la correcta visualización en entornos web móviles.
